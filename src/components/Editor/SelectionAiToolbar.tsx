...
export function SelectionAiToolbar({
  ...
  // Remeasure when the custom-prompt row opens/closes — that changes width
  // a lot, and centering via translate(-50%) used to shove it past the left edge.
  useLayoutEffect(() => {
    if (!visible) return;
  ...
  }, [visible, state.customPromptOpen, state.phase, state.tooLong]);
  ...
  return (
    ...
    >
      ...
      <button
        ...
        disabled={busy || !state.llmReady || state.tooLong}
