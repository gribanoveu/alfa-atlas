...
export const ASSISTANT_SUGGESTIONS: AssistantSuggestion[] = [
...

  {
    ...
    text:
      ...
      "Прочитай скилл method-spec если он доступен и используй его как эталонный пример.\n" +
      "Для формирования разделов предложи пользователю артефакт используя тул requestArtifact с типом http-request.\n" +
  ...
  {
    id: "draft-jira-ticket",
    label: "Задача для Jira",
    hint:
      "Вы описываете, что нужно сделать. Ассистент оформит это задачей для трекера — зачем она нужна, " +
      "что должно измениться, что в границах и что вне, критерии приёмки и Definition of Done — " +
      "и откроет готовое описание редактируемой вкладкой.",
    writes: true,
    mode: "agent",
    inputs: [
      {
        ...
        placeholder: "отправлять документы на подпись прямо из карточки клиента",
        required: true,
        multiline: true,
      },
    ],
    text:
      "Составь задачу для Jira. Что нужно сделать: {{task}}\n" +
      "Заведи её артефактом: пользователь должен получить редактируемую вкладку с готовым " +
      "описанием, а не текст в ответе. Разделы и их порядок возьми из скилла jira-task-description, " +
      "если он доступен: зачем задача, что должно измениться, границы, техническое решение, " +
      "критерии приёмки, Definition of Done, риски, ссылки.\n" +
      "Пиши только то, что есть в описании или прямо из него следует. Раздел, который нечем " +
      "заполнить, оставь пустым, а в конце перечисли, что нужно уточнить. Разметку руками " +
      "не пиши — её соберёт рендерер.",
  },
