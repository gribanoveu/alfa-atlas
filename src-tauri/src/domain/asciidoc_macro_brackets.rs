...
pub fn ensure_macro_attribute_brackets(content: &str) -> String {
    if content.is_empty() {
        return String::new();
    }
    ...
    for line in content.lines() {
        ...
        out.push(close_macros_on_line(line));
    }
...
fn close_macros_on_line(line: &str) -> String {
...
mod tests {
    ...
    fn closes_bare_include_image_and_xref() {
        ...
        assert_eq!(
            ensure_macro_attribute_brackets(src),
    ...
    fn is_idempotent_when_brackets_already_present() {
        let src = "include::request.adoc[]\nimage::x.png[Alt]\nxref:doc.adoc#a[]\n";
        assert_eq!(ensure_macro_attribute_brackets(src), src);
    }
