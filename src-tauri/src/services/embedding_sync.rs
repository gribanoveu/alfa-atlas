...
use crate::domain::embeddings::{SyncPhase, SyncProgress, SyncStats, SyncTrigger};
...
use crate::services::embedding_state::{
    attach_embedding_index, ensure_provider, is_current_index_root, lock_sync_guard,
    BackgroundBacklog,
    BackgroundBacklogSlot, EmbeddingIndexSlot, EmbeddingProviderSlot, EmbeddingSyncGuard,
    IndexWatcherSlot, META_EMBEDDING_DIMENSIONS,
};
...
/// `repoRoot`).
pub(crate) fn direct_dependencies(workspace_index: &WorkspaceIndex, file_id: &FileId) -> Vec<FileId> {
...
pub(crate) fn load_persisted_symbols(store: &IndexStore) -> Result<HashMap<FileId, ReusableFileData>, String> {
...
pub(crate) const BACKGROUND_BATCH_FILES: usize = 25;
...
/// rest of the batch.
#[allow(clippy::too_many_arguments)]
pub(crate) fn sync_backlog_batch(
    repo_index: &RepositoryIndex,
    chunk_index: &ChunkIndex,
    embedding_index: &EmbeddingIndexSlot,
    embedding_provider: &EmbeddingProviderSlot,
...
pub(crate) const INLINE_TIER2_FILE_LIMIT: usize = 25;
...
pub(crate) fn merge_background_backlog(
...
/// it out entirely.
#[allow(clippy::too_many_arguments)]
pub(crate) fn run_background_backlog_sync(
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn ensure_incremental_watcher(
