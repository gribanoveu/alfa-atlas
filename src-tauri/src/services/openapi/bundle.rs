...
use crate::domain::openapi::{OpenApiBundleResult, OpenApiError, RefDiagnostic, SourceRef};
use crate::domain::paths;
use crate::infra::common_spec_assets;

use super::gradle::{detect_gradle_common_params, inject_common_parameters};
...
pub(crate) fn parse_generic(text: &str, ext: &str) -> Result<Value, String> {
    if ext == "json" {
...
pub(crate) fn split_ref(r: &str) -> (&str, &str) {
    match r.split_once('#') {
...
pub(crate) fn ext_fallback_candidates(path: &Path) -> Vec<PathBuf> {
...
pub(crate) fn display_relative(repo_root: &Path, p: &Path) -> String {
...
pub fn is_common_spec_fallback_path(path: &Path) -> bool {
...
pub(crate) fn bundled_common_spec_value() -> Rc<Value> {
...
pub fn load_openapi_bundle(
    ...
    let common_params_enabled = detect_gradle_common_params(repo_root);
    if common_params_enabled {
        inject_common_parameters(&mut document);
    }
...
pub(crate) struct Resolver {
    ...
    pub(crate) file_cache: RefCell<HashMap<PathBuf, Rc<Value>>>,
    /// Fully-inlined result per (file, pointer), so a fragment referenced
    /// from multiple places is only walked once.
    pub(crate) resolved_cache: RefCell<HashMap<(PathBuf, String), Rc<Value>>>,
    /// (file, pointer) pairs currently being resolved, for cycle detection.
    pub(crate) in_progress: RefCell<HashSet<(PathBuf, String)>>,
    pub(crate) diagnostics: RefCell<Vec<RefDiagnostic>>,
    /// Куда в исходных файлах ведёт каждая успешно разрешённая граница
    /// `$ref` — см. [`SourceRef`].
    pub(crate) sources: RefCell<Vec<SourceRef>>,
    ...
    pub(crate) cycle_components: RefCell<HashMap<(PathBuf, String), String>>,
    /// Имена, уже занятые в `components/schemas` входного документа, — чтобы
    /// вынесенная схема не затёрла объявленную вручную.
    pub(crate) taken_component_names: RefCell<HashSet<String>>,
    ...
    pub(crate) enable_ref_fallback: bool,
}
...
pub(crate) fn component_base_name(file: &Path, fragment: &str) -> String {
    let raw = fragment
        .rsplit('/')
...
impl Resolver {
    ...
    pub(crate) fn component_name_for(&self, key: &(PathBuf, String)) -> String {
    ...
    pub(crate) fn inject_cycle_components(&self, document: &mut Value) {
    ...
    pub(crate) fn record_source(&self, doc_pointer: &str, target_file: &Path, fragment: &str) {
    ...
    pub(crate) fn load_file(&self, path: &Path) -> Result<Rc<Value>, ()> {
    ...
    pub(crate) fn resolve_ref(&self, ref_str: &str, current_file: &Path, doc_pointer: &str) -> Value {
    ...
    pub(crate) fn walk(&self, value: &Value, current_file: &Path, doc_pointer: &str) -> Value {
        match value {
            Value::Object(map) => {
