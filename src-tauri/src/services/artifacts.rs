...
pub fn update_agent(
    ...
    let record = artifact_store::stamp_updated(ArtifactRecord {
        ...
        // Wholesale: a rewrite replaces the content, and «убери этот риск»
...
pub fn save(incoming: ArtifactRecord) -> Result<ArtifactRecord, ArtifactError> {
    ...
    let record = artifact_store::stamp_updated(ArtifactRecord {
        ...
        // Wholesale, like `update_agent`: the builder round-trips the whole
        // record and its content is the only thing it edits.
