...
use crate::commands::chat_events::{chat_event_sink, NO_CHAT_TURN, RATE_LIMIT_CHANGED_EVENT};
...
pub async fn llm_chat_stream(
    app: AppHandle,
    provider_id: String,
    // Identifies this turn on every event it emits, so a listener can drop
    // what belongs to a turn it is not showing — see `chat_event_sink`.
    // Generated by the frontend, which needs it before this promise
    // resolves; a resume must echo back the id of the turn it continues.
    turn_id: String,
    messages: Vec<LlmMessage>,
    todos: Vec<Task>,
    active_file_path: Option<String>,
...
pub async fn llm_chat_stream_resume(
    app: AppHandle,
    provider_id: String,
    // The id of the turn being continued — the same one its original
    // `llm_chat_stream` call was given, so the resumed rounds keep arriving
    // under the identity the frontend is already listening for.
    turn_id: String,
