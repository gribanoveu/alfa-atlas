...
Актуальное описание того, что уже работает в **Alfa Atlas** (Tauri + React + Rust), и как это устроено.  
Целевые бизнес-требования продукта (vision / backlog) — в [business-requirements/](./business-requirements/).

Идентификатор приложения: `com.eugene.alfa-atlas`. Стек: Tauri v2, React 19, TypeScript, Monaco, bun, Rust.
...
| Область | Поведение |
|---------|-----------|
| **TopBar** | Бренд `Alfa Atlas`, выпадающие меню (Файл…Справка), чипы репозитория / ветки. Имя репозитория — последний сегмент `repoRoot`; ветка — через `git2` (`get_git_branch`). |
