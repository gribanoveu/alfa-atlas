...
The provider layer a future chat feature will call into — config, credentials, an OpenAI-compatible HTTP client, and a Settings UI, all built and unit-tested, but not yet wired into `execute_tool`/`ToolCall` or any chat surface. Deliberately mirrors the Embeddings layer's own `domain`/`infra`/`services`/`commands` split (see above) rather than inventing new conventions.

`domain/llm.rs`: `LlmProvider` trait (`chat`/`list_models`, synchronous — same `spawn_blocking` reasoning as `EmbeddingProvider`, this project's `tokio` has no `net` feature), `ChatRequest`/`ChatResponse`/`LlmMessage`/`LlmToolDefinition`/`LlmToolCall` (protocol-agnostic; `LlmToolCall::arguments` stays a raw JSON string, matching the wire reality — parsing it into concrete tool args is `services::ai_tools`'s job once the tool-execution loop exists), `LlmProviderConfig` (a persisted override or standalone custom entry), `LlmSettings` (`AppSettings.llm`), `LlmProviderPreset` (one compiled-in manifest entry), `ResolvedLlmProvider` (the merged view everything else actually uses). Deliberately no `LlmProviderKind` protocol enum — every provider today speaks OpenAI-compatible; `infra::llm_providers::provider_for` is the one dispatch point a second protocol would extend.

**System providers are build-time data, not code**: `assets/llm/system_llm_providers.json` (loaded by `infra/llm_provider_manifest.rs` via `include_str!`, same embedding-over-Tauri-bundle-resources rationale as `common_spec_assets.rs`/`dictionary_assets.rs`) ships one preset today — `alfagen` (`https://alfagen.moscow.alfaintra.net/continue-dev`), with `trustedCertPem: null` (no real certificate supplied yet — see below). A downstream fork/rebrand edits or empties this one file and rebuilds; no `.rs` change needed either way.
...
**IPC surface** (`commands/llm.rs`, 9 commands): `llm_get_settings`/`llm_set_settings`, `llm_list_providers` (the merged system+custom list the Settings picker renders), `llm_upsert_provider`/`llm_remove_provider`, `llm_set_api_key`/`llm_has_api_key` (write-only key, mirrors the embedding credentials commands), `llm_list_models` (async, live `/models` call), and `llm_test_connection` (async — calls `list_models` and reports the count, or the error). `llm_test_connection` exists specifically so this whole stack — config, credentials, TLS, HTTP, response parsing — is verifiable end-to-end (a "Проверить соединение" button in Settings) without a chat UI or the tool-execution loop existing yet; it deliberately never calls `chat` (no completion spent, no dependency on a resolvable model). `LlmProviderSlot` caches the constructed provider keyed by `(resolved, api_key)`, same shape as `EmbeddingProviderSlot`.

**UI**: a "LLM" tab in Settings (`LlmTab.tsx`/`useLlmSetup.ts`, mirroring `EmbeddingsTab.tsx`/`useEmbeddingSetup.ts`) — provider list with "встроенный"/"свой" badges, base URL (editable only for custom providers), a model dropdown (populated by `llm_list_models`, with an "Авто (первая доступная)" option), a certificate override textarea with a "сбросить к встроенному" action, API-key input (never pre-filled), and the test-connection button.
...
- The LLM client (config, credentials, provider registry, OpenAI-compatible HTTP layer — see "LLM client" above) now exists, but there's still no chat UI, no streaming, and no tool-execution loop wiring it into `ToolCall`/`execute_tool` — `LlmProvider::chat` is implemented and unit-tested but not invoked from any command yet (`llm_test_connection` only calls `list_models`).
- No write/edit tool.
