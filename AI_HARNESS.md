...
**The tool-execution boundary is reachable from the frontend via one IPC command (`ai_execute_tool`); a standalone Repository Index + Chunk Index exist alongside it; and an Embedding Service (local BGE-M3 or a remote OpenAI-compatible API, backed by a `usearch` vector index) now turns chunks into searchable vectors and persists both the vectors and the chunk/file metadata needed to reload them — but no LLM is wired up and no chat UI exists yet.** This is groundwork for a future AI harness (an agent loop that calls an LLM with tool-calling) — specifically the trust boundary that decides *which files* and *which operations* that harness is allowed to touch, a per-file structural index, a semantic-chunk index, and now the embedding layer a future semantic-search/RAG tool will query. The Rust side (`src-tauri/src`) is unit-tested throughout; the IPC command is additionally verified end-to-end (see "IPC surface" below) but has no caller anywhere in the app yet. Repository Index and Chunk Index still have no caller at all (not even IPC) beyond `embedding_sync` rebuilding them internally. The Embedding Service is the first layer in this file with real UI: a "Эмбеддинги" tab in Settings and a setup checklist in the RightDock "Ассистент" panel, both driving the same 8 `embedding_*` IPC commands and a live `embedding:sync-progress` event stream — but nothing calls `embedding_sync` automatically yet (no file-watcher), and there is still no semantic-search command/UI consuming the resulting vectors.

**Memory/persistence architecture (added after the initial pass below was written):** `ChunkIndex` no longer holds chunk text resident in memory — only `ChunkMetadata` (byte offsets + hashes); text is read on demand from source files via `services::chunk_text::resolve_text`, and only for chunks a caller actually needs (a sync's pending/changed set, or — once it exists — a search's top-K results). `EmbeddingRecord` no longer duplicates the embedding vector (it only ever lived redundantly alongside `usearch`'s own copy). Both `ChunkIndex` and `EmbeddingIndex` mirror to a per-project SQLite store (`infra::index_store::IndexStore`, at `{index_root}/.atlas/index/chunks.db`) plus a `vectors.usearch` file, so a restart reloads persisted state instead of re-walking/re-embedding from scratch, and `embedding_sync` re-chunks/re-embeds only files whose content hash actually changed since the last sync — see "Chunk Index" and "Embeddings" below for the details, and note that **the index is per-`AiAccessMode`**: `DocsOnly` and `FullRepo` resolve to different `index_root`s (`docsRoot` vs `repoRoot`), so each has its own separate persisted store — switching the toggle does not share or invalidate the other mode's index, but a mode that has never been synced shows as empty until its own first `embedding_sync`.
...
commands/embeddings.rs             — 8 embedding_* IPC commands (incl. embedding_index_status), EmbeddingIndexSlot, IndexStoreSlot, emits embedding:sync-progress
src/lib/embeddings.ts              — typed wrappers + listenModelDownloadProgress()/listenSyncProgress()
src/hooks/useEmbeddingSetup.ts     — shared state (config/modelStatus/lastSync/indexStatus/syncProgress) consumed by EmbeddingsTab and AssistantPanel
...
**Text is not resident in `ChunkIndex`.** Storing every chunk's text (up to `DEFAULT_MAX_CHUNK_BYTES`, 16KB, each) for a whole indexed tree would make resident memory scale with the total size of the indexed text, not with chunk count — the same duplication `IndexedFile` already avoids one layer down. `services::chunk_text::resolve_text(repo_root, &ChunkMetadata)` reads `[start_byte..end_byte)` straight off the source file instead, first checking that the file's current content still hashes to `metadata.file_hash` (`ChunkTextError::Stale` if not — the file changed or was deleted since indexing) before trusting the byte range. `ChunkIndex::get_with_text` wraps this for callers that need both metadata and text in one call. The practical effect: `EmbeddingIndex::sync` only ever reads text for chunks that are new or changed (see "Embeddings"), and a future semantic-search tool would only read it for a query's top-K results — never for the whole corpus.

**Persistence**: `ChunkIndex`'s metadata is mirrored to the `chunks`/`files` tables of a per-project `infra::index_store::IndexStore` (SQLite, `{index_root}/.atlas/index/chunks.db`) — `ChunkMetadata` in, `ChunkMetadata` out (`IndexStore::load_all_chunks`), no text. `ChunkIndex::load_metadata` bulk-repopulates the resident `DashMap` from that on a cold start (a new `index_root` this process hasn't seen yet — first sync since app launch, or a project/access-mode switch), so the metadata a diff needs (`file_hash_for`, `file_ids`) survives a restart without a repo rescan. `services::index_store_ensure::open_for(index_root)` is the guard in front of this: it compares the store's persisted `chunk_version`/`index_version`/`index_root` meta rows against the running binary's `CHUNK_VERSION`/`repo_index::INDEX_VERSION` and the freshly-resolved `index_root`, and wipes the store (and the sibling `vectors.usearch`) rather than risk loading metadata produced by an incompatible chunking algorithm or a different access-mode root.
...
`IndexStoreSlot` (`Mutex<Option<(PathBuf, Arc<IndexStore>)>>`) is the third such lazily-attached slot — one SQLite connection per `index_root`, shared by `ChunkIndex`'s cold-start reload and `EmbeddingIndex`'s persistence. `commands::embeddings::attach_index_store`/`attach_embedding_index` are the two helpers both `embedding_sync` and `embedding_index_status` build on to reach this state without duplicating the attach-or-reload logic.

**IPC surface** (`commands/embeddings.rs`, 8 commands): `embedding_get_config`/`embedding_set_config`, `embedding_set_remote_api_key`/`embedding_has_remote_api_key` (write-only key, mirrors `git_save_credentials`), `embedding_model_status`, `embedding_download_model` (async, streams `embedding:model-download-progress`), `embedding_sync` (async — full `RepositoryIndex.build()` rescan every call, but re-chunks only files whose hash changed since `ChunkIndex` last saw them via `ChunkIndex::file_hash_for`, then `EmbeddingIndex::sync`; emits `embedding:sync-progress` events — `phase: "chunking"|"embedding"` — while running; returns `SyncStats{embedded, skipped_unchanged, removed}`), and **`embedding_index_status`** (async, read-only — attaches/cold-start-reloads the same state `embedding_sync` would use, but never walks the repo or calls the embedding provider beyond resolving its dimension count; returns `EmbeddingIndexStatus{synced, embeddedCount}` derived from the resident/persisted index itself, not from whether a sync happened to run earlier in this process). Frontend wrappers in `src/lib/embeddings.ts` (incl. `listenSyncProgress`), shared state in `src/hooks/useEmbeddingSetup.ts` (`indexStatus`, refreshed on mount and after every `sync()`, is what lets the UI show real "already built" state across a component remount instead of resetting to "not yet synced").
...
- `RepositoryIndex::build()` still does a full walk + re-hash + re-parse of every file on every `embedding_sync` call — cheap relative to embedding inference (no network/ONNX), so not yet worth diffing further, but there's no `mtime`/size pre-filter to skip re-hashing an untouched file either. What *is* incremental: `ChunkBuilder`/`ChunkIndex` re-chunking (skipped per-file via `file_hash_for`) and `EmbeddingIndex`'s own per-chunk `chunk_hash` diff.
...
- No actual file-watcher-driven incremental rebuild — `embedding_sync` (and, underneath it, `ChunkBuilder::build_file` + `ChunkIndex::replace_for_file`) exist in the shape a watcher will call, and are already incremental when invoked, but nothing triggers a sync on a file-change event yet; it only runs when a user clicks "Синхронизировать" in Settings or the Assistant panel. `RepositoryIndex`/`ChunkIndex`/`EmbeddingIndex` are not subscribed to `services::file_watcher::FileWatcher` (which today only drives `WorkspaceIndex`, the editor's own document cache) — they go stale on disk changes until the next manual sync.
- No bounded in-memory text cache — `chunk_text::resolve_text` re-reads a file from disk on every call; fine for a sync's pending set or a future search's top-K, but repeated overlapping lookups within one session would re-read the same file each time. Not added speculatively; `resolve_text` is a single seam a `moka`-style cache could sit behind later if profiling shows it matters.
- No semantic-search IPC command or UI — `EmbeddingIndex::search` exists and is unit-tested, but nothing calls it; this stage only builds (and now persists) the index, not a way to query it.
