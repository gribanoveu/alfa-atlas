...
The provider layer the assistant chat feature calls into — config, credentials, an OpenAI-compatible HTTP client (streaming and non-streaming), a Settings UI, and a real chat surface in the right dock. Deliberately mirrors the Embeddings layer's own `domain`/`infra`/`services`/`commands` split (see above) rather than inventing new conventions. Not yet wired: any tool-execution loop into `execute_tool`/`ToolCall` — chat today is plain conversation, the model can't search or read the repo.

`domain/llm.rs`: `LlmProvider` trait (`chat`/`chat_stream`/`list_models`, synchronous — same `spawn_blocking` reasoning as `EmbeddingProvider`, this project's `tokio` has no `net` feature; `chat_stream` takes a caller-supplied `&dyn Fn(&str)` delta callback and returns the full accumulated reply once the stream ends), `ChatRequest`/`ChatResponse`/`LlmMessage`/`LlmToolDefinition`/`LlmToolCall` (protocol-agnostic; `LlmToolCall::arguments` stays a raw JSON string, matching the wire reality — parsing it into concrete tool args is `services::ai_tools`'s job once the tool-execution loop exists), `ModelLimit` (`context`/`output` token counts — informational only, not enforced anywhere in this client; merges through the same manifest-preset/settings-override layers as `model`), `LlmProviderConfig` (a persisted override or standalone custom entry), `LlmSettings` (`AppSettings.llm`), `LlmProviderPreset` (one compiled-in manifest entry), `ResolvedLlmProvider` (the merged view everything else actually uses). Deliberately no `LlmProviderKind` protocol enum — every provider today speaks OpenAI-compatible; `infra::llm_providers::provider_for` is the one dispatch point a second protocol would extend.
...
**Chat UI** (`AssistantPanel.tsx`/`useLlmChat.ts`, in the right dock): a real, working conversation, not a placeholder shell. Gated on **LLM readiness** (an active/first-resolved provider with a saved API key) rather than the embedding provider — plain chat doesn't touch embeddings at all this iteration (no tool-calling/search wired in yet), so an unconfigured embedding provider only shows a non-blocking info note, parallel to the existing "index still building" note. `useLlmChat` owns one conversation's state (`messages`/`sending`/`error`/an estimated `contextTokens` count), subscribes to the delta event once for its lifetime, and builds `[system prompt, ...prior turns, new message]` on every send. The panel also has its own model picker (the same `.clone-select` dropdown pattern as `LlmTab.tsx`'s, writing through the same `updateProviderConfig` so Settings and the chat panel share one source of truth) and an estimated context-window usage bar under the message list — `estimateTokenCount` (`src/lib/tokens.ts`) is a `~4 chars/token` heuristic, **not** a real per-model tokenizer (none is available client-side, and no provider here returns usage counts), shown only when the active provider's `limit.context` is known. `src/lib/assistantConfig.ts` centralizes the handful of tunable constants shared across `LlmTab.tsx`/`AssistantPanel.tsx`/`useLlmChat.ts` — `buildAssistantSystemPrompt(mode: AiAccessMode)` (built per-request rather than a frozen string, so the date/timezone line and the docs-only/full-repo mode line — sourced from the same `useAiAccessMode` toggle `AssistantPanel` renders — never go stale mid-session; the actual tool-calling/project-context injection this prompt anticipates is still not wired), the "Авто" model-picker sentinel value/label, the compose box's row count, and the context-bar warning threshold — rather than leaving them scattered or duplicated per component. Deliberate scope limits for this increment: no markdown rendering (plain text, `white-space: pre-wrap`), no stream cancellation/abort, no conversation persistence across app restarts, no context-window truncation of long histories.
...
- The LLM client and a real streaming chat UI now exist (see "LLM client" above), but there's still no tool-execution loop wiring the assistant into `ToolCall`/`execute_tool` — chat is plain conversation only, the model can't search or read the repo yet. `LlmProvider::chat` (the non-streaming path) is implemented and unit-tested but still not invoked from any command (`llm_test_connection` only calls `list_models`; the chat UI always uses `chat_stream`).
- No markdown rendering in the chat UI, no stream cancellation/abort, no conversation persistence across app restarts, no context-window truncation of long histories — see "Chat UI" under "LLM client" above for the full list of this increment's deliberate scope limits.
- No write/edit tool.
