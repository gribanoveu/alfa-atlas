...
`INDEX_VERSION` (currently `1`) — bumped whenever indexer behavior changes in a way that would change output for the same input — is exactly the staleness signal `services::index_store_ensure::open_for` now checks against a persisted store's `index_version` meta row before trusting it (see "Chunk Index" below); `RepositoryIndex` itself still rebuilds via a full walk on every `embedding_sync` (still no `mtime`/size pre-filter to skip re-hashing an untouched file), but no longer re-parses every file's symbols to do it: a `symbols` table (mirroring `chunks`/`embeddings`) now persists each file's `Vec<Symbol>` alongside `files.file_hash`, and `RepositoryIndex::build_reusing_symbols(repo_root, persisted)` reuses a file's already-known symbols — whether resident from earlier this session or loaded from that table — whenever its current content hash still matches, skipping the tree-sitter/pulldown-cmark parse entirely. `build()` itself (used directly by tests and anything that doesn't have persisted data to offer) gets the same in-session reuse for free via an internal snapshot of whatever's already resident before it clears and rewalks.
